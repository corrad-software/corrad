<template>
  <div class="p-6">
    ...
    <div class="flex items-center space-x-4 mb-4">
      ...
      <div class="flex-1">
        <label class="text-sm font-bold mb-1 block">Session ID</label>
        <input v-model="sessionId" type="text" placeholder="Enter Session ID" class="border rounded-md w-full" />
      </div>
      ...
      <table class="table-auto w-full border-collapse border">
        <thead class="bg-blue-100">
          <tr>
            ...
            <th class="border px-4 py-2">JD Dateline</th>
            <th class="border px-4 py-2">Skillset Dateline</th>
        ...
        <tbody>
          <tr v-for="(item, i) in filteredData" :key="i" class="text-center">
            ...
            <td class="border px-4 py-2">{{ item.jdDateline }}</td>
            <td class="border px-4 py-2">{{ item.skillsetDateline }}</td>
            <td class="border px-4 py-2">{{ item.pppDateline }}</td>
            <td class="border px-4 py-2">{{ item.ppkDateline }}</td>
            <td class="border px-4 py-2">{{ item.status }}</td>
            <td class="border px-4 py-2">
              <rs-button variant="primary" @click="updateItem(item)">Update</rs-button>
            </td>
          </tr>
        </tbody>
      </table>
...
const data = ref([
  {
    title: '2024 CBTM Review',
    startDate: '26 Sep 2024',
    endDate: '31 Dec 2024',
    jdDateline: '31 Dec 2024',
    skillsetDateline: '31 Dec 2024',
    pppDateline: '26 Sep 2024',
    ppkDateline: '26 Sep 2024',
    status: 'Active',
  },
  {
    ...
    endDate: '31 Dec 2024',
    jdDateline: '30 Sep 2024',
    skillsetDateline: '30 Sep 2024',
...
const filteredData = computed(() => {
  ...
  // Simple filtering logic for "title" or "sessionId"
  if (searchText.value) {
...
// Update Action for each row
const updateItem = (item) => {
  console.log('Updating item:', item);
  // Here you can add logic to navigate to an update page or open a modal for editing the item
};
</script>
