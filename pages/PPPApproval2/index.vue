<template>
  <div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-6">PPP Approval</h1>
    ...
    <table class="table-auto w-full border border-gray-300 mb-4">
      ...
      <tbody>
        <tr v-for="(record, index) in paginatedData" :key="record.id" class="border-b">
          ...
          <td class="p-2 text-center">
            <button @click="viewApproval(record.id)" class="btn-view">View</button>
          </td>
        </tr>
      </tbody>
    </table>
...
const totalPages = computed(() => Math.ceil(approvals.value.length / pageSize))
...
// View button action to navigate to the detailed approval page
const viewApproval = (id) => {
  router.push(`/PPPApproval2Staff?id=${id}`)
}
</script>
