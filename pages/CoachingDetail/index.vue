<template>
  <div>
    ...
    <div class="flex items-center justify-center mb-6">
      <div class="flex-1">
        <p><strong>Staff ID:</strong> 5525</p>
        <p><strong>Staff Name:</strong> Mohd Aamir Adeeb bin Abdul Rahim</p>
        <p><strong>Email:</strong> aamir@uum.edu.my</p>
        <p><strong>Designation:</strong> Pensyarah Universiti (DS51)</p>
        <p><strong>Department:</strong> Sains Kuantitatif</p>
      </div>
      <div class="ml-4">
        <!-- Staff Photo -->
        <img src="https://via.placeholder.com/100" alt="Staff Photo" class="rounded-full border" />
      </div>
    </div>
    ...
    <div class="flex justify-center gap-8">
      ...
      <div>
        ...
        <FormKit type="text" v-model="availableSearch" placeholder="Keyword [Name / Staff No]" class="mb-2 w-full" />
        <select multiple v-model="selectedAvailableStaff" class="h-64 w-full border">
          <option v-for="(staff, index) in filteredAvailableStaff" :key="index" :value="staff">
      ...
      <div>
        ...
        <FormKit type="text" v-model="selectedSearch" placeholder="Keyword [Name / Staff No]" class="mb-2 w-full" />
        <select multiple v-model="selectedSelectedStaff" class="h-64 w-full border">
          <option v-for="(staff, index) in filteredSelectedStaff" :key="index" :value="staff">
            {{ staff.name }} [{{ staff.staffNo }}]
          </option>
        </select>
      </div>
...
const availableStaff = ref([
  { name: "Faizul bin Manaf", staffNo: 3806 },
  { name: "Ku Nurhanim binti Ku Baduin", staffNo: 5496 },
  { name: "Mahani Binti Ahmad", staffNo: 5563 },
...
<style scoped>
/* Add some spacing for the dual list box */
select {
  padding: 8px;
  font-size: 14px;
}
