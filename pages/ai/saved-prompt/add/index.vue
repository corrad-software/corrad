...
const submitForm = async (formData) => {
  try {
    const { data } = await useFetch("/api/ai/saved-prompt/add", {
      method: "POST",
      body: formData,
    });
    ...
    if (data.value && data.value.statusCode === 200) {
      ...
      $swal.fire({
        title: "Error",
        text:
          data.value?.message || "An error occurred while saving the prompt",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    $swal.fire({
      title: "Error",
      text: "An error occurred while saving the prompt",
      icon: "error",
    });
  }
};
